...
NgetFinalResultsNonBlocking <- function(obj, convert = TRUE){
  ...
  nc <- obj@nc
  ans <- xml.rpc(url = nc@url, method = "getFinalResultsNonBlocking", .args = list(jobnumber = jobnumber, password = password), .convert = FALSE, .opts = nc@curlopts, .curl = nc@curlhandle)
  if(convert){
